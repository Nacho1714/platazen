...
$page = $_GET['s'] ?? 'dashboard'; // Con operador de fusión de null (null coalesce). Si $_GET['s'] es "null" entonces $vista = 'home'
...
foreach ($whitelist as $index => $directory) {
...
if ($requiresAuth && (!$authManager->checkAuth() || !$authManager->checkAdmin())) {
...
$authPermission = $authManager->checkAuth() && $authManager->checkAdmin() && $directory !== 'auth' ? true : false;
...
<head>
	...
	<title><?= $routeConfig['title'] ?? '' ?> :: Panel de Administración</title>
			...
			<ul class="list-unstyled component m-0" id="sidebar-menu">
				...
				<li class="dropdown">
					<a href="#collapseExample" class="dropdown-toggle" data-bs-toggle="collapse" role="button" aria-expanded="false" aria-controls="collapseExample">
						<i class="material-icons">aspect_ratio</i>Ordenes
					</a>
					<ul class="collapse" id="collapseExample">
						<li><a href="index.php?s=dashboard&sb=orders">TODO</a></li>
